...
> 版本: v3
> 变更: 新增 reset 命令用于重置工作流状态

## 1. 一句话摘要

IncSpec 是一个纯 ES Modules 的 CLI 工具，通过 `index.mjs` 入口解析命令行参数并路由到 15 个命令模块，命令模块依赖 7 个核心库实现配置管理、工作流状态、规范文件操作和终端交互。
...
sequenceDiagram
    ...
    alt 工作流操作
        ...
        Workflow->>Terminal: [S4] formatLocalDateTime(new Date())
    ...
    alt 配置操作
        ...
        Config->>Terminal: [S4'] formatLocalDate(new Date())
        Terminal-->>Config: [S5'] "2025-12-20"
        Config-->>Cmd: [S6'] 配置内容
    end
    
    alt list 命令
        Cmd->>Terminal: [S3''] formatLocalDateTime(spec.mtime)
...
sequenceDiagram
    ...
    alt --baseline 选项
        ...
        alt 无活跃工作流
            ...
            Workflow->>Terminal: [S10a] formatLocalDateTime(new Date())
            Workflow->>FS: [S11a] 写入 WORKFLOW.md
        else 有未完成工作流
            Analyze->>Terminal: [S9b] confirm() 询问归档
            Terminal-->>Analyze: [S10b] 用户选择
        end
        
        Analyze->>Spec: [S12] getNextVersion('baselines', module)
        Spec->>FS: [S13] 扫描版本号
        Spec-->>Analyze: [S14] 下一版本号
        Analyze->>Workflow: [S15] updateStep(1, IN_PROGRESS)
        Analyze->>User: [S16] 显示使用说明
    end
```
...
sequenceDiagram
    participant User as 用户
    participant Reset as reset.mjs
    participant Config as config.mjs
    participant Workflow as workflow.mjs
    participant Spec as spec.mjs
    participant Terminal as terminal.mjs
    ...
    User->>Reset: [S1] incspec reset
    Reset->>Config: [S2] ensureInitialized(cwd)
    Config->>FS: [S3] findProjectRoot()
    FS-->>Config: [S4] projectRoot
    Config-->>Reset: [S5] projectRoot 或抛错
    
    Reset->>Workflow: [S6] readWorkflow()
    Workflow->>FS: [S7] 读取 WORKFLOW.md
    FS-->>Workflow: [S8] 工作流状态
    
    alt 无活跃工作流
        Reset->>Terminal: [S9a] printInfo("当前无活跃工作流")
        Reset->>User: [S10a] 退出
    else 有活跃工作流
        Reset->>Reset: [S9b] collectWorkflowOutputs()
        Reset->>Reset: [S10b] extractModuleName(workflowName)
        loop 每个产出文件
            Reset->>Spec: [S11] archiveSpec(path, true, moduleName)
            Spec->>FS: [S12] 移动到 archives/YYYY-MM/{module}/
        end
        Reset->>Terminal: [S13] printSuccess("已归档 N 个文件")
        Reset->>Workflow: [S14] archiveWorkflow()
        Workflow->>FS: [S15] 清空 WORKFLOW.md 状态
        Reset->>Terminal: [S16] printSuccess("工作流已重置")
        Reset->>User: [S17] 完成
    end
```
...
graph TB
    ...
    RESET --> CONFIG & WORKFLOW & SPEC & TERMINAL
    SYNC --> CURSOR & CLAUDE & TERMINAL
    HELP --> WORKFLOW & TERMINAL
    
    CONFIG --> TERMINAL & FS & PATH
    WORKFLOW --> CONFIG & TERMINAL & FS & PATH
    SPEC --> CONFIG & FS & PATH
    TERMINAL --> READLINE
...
graph LR
    subgraph "核心库 lib/"
        TERMINAL[terminal.mjs<br/>终端 I/O<br/>+ formatLocalDateTime<br/>+ formatLocalDate]
    ...
    WORKFLOW -->|formatLocalDateTime| TERMINAL
    WORKFLOW -->|getTemplatesDir| CONFIG
    CONFIG -->|formatLocalDate| TERMINAL
...
| - | reset | 归档产出并清空状态 | `incspec/archives/YYYY-MM/{module}/` |
...
project.md (YAML frontmatter)
    ...
    ├── tech_stack: 技术栈数组
    ├── source_dir: 源代码目录
    └── created_at: 创建日期 (formatLocalDate)
         │
         ▼
...
| 函数 | 行号 | 用途 |
|------|------|------|
| `findProjectRoot(startDir)` | 39-53 | 向上查找 incspec/ 目录 |
| `isInitialized(cwd)` | 65-69 | 检查是否已初始化 |
| `ensureInitialized(cwd)` | 193-201 | 确保已初始化，否则抛错 |
| `parseFrontmatter(content)` | 74-110 | 解析 YAML frontmatter |
| `readProjectConfig(projectRoot)` | 127-140 | 读取项目配置 |
| `createIncspecStructure(projectRoot, config)` | 174-191 | 创建目录结构 |
| `getTemplatesDir()` | 29-35 | 获取模板目录路径 |

### 5.2 workflow.mjs

| 函数 | 行号 | 用途 |
|------|------|------|
| `readWorkflow(projectRoot)` | 119-129 | 读取工作流状态 |
| `writeWorkflow(projectRoot, workflow)` | 170-174 | 写入工作流状态 |
| `startWorkflow(projectRoot, name)` | 196-224 | 创建新工作流 |
| `updateStep(projectRoot, stepNumber, status, output)` | 230-255 | 更新步骤状态 |
| `archiveWorkflow(projectRoot)` | 275-295 | 归档当前工作流 |
| `getWorkflowProgress(workflow)` | 316-333 | 计算完成进度 |
| `parseWorkflow(content)` | 72-117 | 解析 WORKFLOW.md |
...
| `formatLocalDateTime(date)` | 375-383 | 格式化本地日期时间 (YYYY-MM-DD HH:mm) |
| `formatLocalDate(date)` | 390-396 | 格式化本地日期 (YYYY-MM-DD) |

### 5.5 reset.mjs (新增)

| 函数 | 行号 | 用途 |
|------|------|------|
| `resetCommand(ctx)` | 66-108 | reset 命令入口，重置工作流状态 |
| `extractModuleName(workflowName)` | 17-32 | 从工作流名称提取模块名 |
| `collectWorkflowOutputs(projectRoot, workflow)` | 50-63 | 收集工作流产出文件列表 |
| `getOutputDirForStepIndex(stepIndex)` | 35-48 | 根据步骤索引获取输出目录 |
...
6. **本地时间**: 使用 `formatLocalDateTime()` / `formatLocalDate()` 生成本地时间，而非 UTC
...
|------|------|----------|
| v1 | 2025-12-20 | 初始基线 |
| v2 | 2025-12-20 | 新增 `formatLocalDateTime()`, `formatLocalDate()` 函数，修复 UTC 时间显示问题 |
| v3 | 2025-12-20 | 新增 `reset` 命令 (别名 `rs`)，用于重置工作流状态并归档已有产出 |
...
- `reset` 命令可快速重置工作流，自动归档产出文件到 archives 目录
- 所有时间戳使用本地时间格式化，符合用户预期
